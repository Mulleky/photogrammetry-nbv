...
class Phase2ControllerNode(Node):
    ...
    def _declare_parameters(self) -> None:
        params = [
            ...
            ('knn_distance_threshold', 0.05), ('knn_k', 5), ('knn_percentile', 95.0),
    ...
    def _run(self) -> None:
        ...
        if self.phase == Phase2State.SCORE_NEXT_VIEW:
            # Export metrics first so kNN stopping criterion can use them
            ...
            if self._should_stop(snapshot):
                self.get_logger().info(
                    f'Stopping criterion met: {self.stopping_criterion} '
            ...
            context = ScoreContext(
                ...
                colmap_workspace=str(self.paths.colmap_dir),
            )
    ...
    def _should_stop(self, snapshot) -> bool:
        """Check whether the mission should stop based on the configured criterion."""
        # Hard safety cap
        ...
        if criterion == 'knn_distance':
            ...
            if not knn:
                return False
            metric_val = knn.get('percentile_knn_distance', float('inf'))
            return metric_val <= self.knn_distance_threshold
        ...
        if criterion == 'both':
            ...
            if knn:
                metric_val = knn.get('percentile_knn_distance', float('inf'))
                knn_done = metric_val <= self.knn_distance_threshold
